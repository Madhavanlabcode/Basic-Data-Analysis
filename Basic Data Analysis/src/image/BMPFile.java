...
public class BMPFile extends Component {
    ...
    //Written by Dan Walkup to make the writing faster if there is no pad.
    private void writeBitmapPadZero()
    {
    	byte[] bytes = new byte[bitmap.length*3];
        int size;
    	int j;
        int i;
        int rowCount;
    	...
    	try {
            ...
            for (j = 0; j < size+1; j++) {
                ...
                bytes[3*j] = (byte) (value & 0xFF);
                bytes[3*j + 1] = (byte) ((value >> 8) & 0xFF);
                bytes[3*j + 2] = (byte) ((value >> 16) & 0xFF);
                if (rowCount == biWidth) {
                    rowCount = 1;
                    rowIndex = lastRowIndex - biWidth;
                    lastRowIndex = rowIndex;
